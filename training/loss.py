...
class MultitaskLoss(torch.nn.Module):
    ...
    - Inverse rendering loss (albedo, metallic, roughness, normal, shading)
    """
    def __init__(self, camera=None, depth=None, point=None, track=None, inverse=None, **kwargs):
        super().__init__()
        ...
        # Inverse rendering loss (NEW)
        self.inverse_loss = None
        if inverse is not None:
            from inverse_loss import InverseRenderingLoss
            self.inverse_loss = InverseRenderingLoss(**inverse)
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        # Inverse rendering loss (NEW)
        if self.inverse_loss is not None:
            # Check if any inverse GT is present in the batch
            inverse_gt_keys = [k for k in batch if k.startswith("gt_")]
            if inverse_gt_keys:
                inv_loss_dict = self.inverse_loss(predictions, batch)
                inv_total = inv_loss_dict.get("loss_inv_total", 0)
                total_loss = total_loss + inv_total
                loss_dict.update(inv_loss_dict)

        # If total_loss is still a float, convert it to a valid tensor to avoid 'int object has no attribute item'
        if not isinstance(total_loss, torch.Tensor) or not total_loss.requires_grad:
            # Try to grab a tensor from predictions to get the correct device and calculation graph
            anchor = None
            for k, v in predictions.items():
                if isinstance(v, torch.Tensor) and v.numel() > 0 and v.requires_grad:
                    anchor = v
                    break
            
            if anchor is not None:
                total_loss = total_loss + anchor.mean() * 0.0
            else:
                # Fallback purely on device info if no predictions tensor was found
                dev = batch["images"].device if "images" in batch else "cpu"
                total_loss = total_loss + torch.zeros((), device=dev, requires_grad=True)
