...
class ComposedDataset(Dataset, ABC):
    ...
    def __init__(self, dataset_configs: dict, common_config: dict, **kwargs):
        ...
        for baseset_dict in dataset_configs:
            ...
            # We create a copy without 'weight' to avoid passing it to the base dataset constructor
            w = baseset_dict.get('weight', 1.0)
            weights.append(float(w))
            
            baseset_cfg = {k: v for k, v in baseset_dict.items() if k != 'weight'}
            baseset = instantiate(baseset_cfg, common_conf=common_config)
            base_dataset_list.append(baseset)

        # Use custom concatenation class that supports tuple indexing and weighted sampling
        self.base_dataset = TupleConcatDataset(base_dataset_list, common_config, weights=weights)
    ...
    def __getitem__(self, idx_tuple):
        ...
        # Convert other data to tensors with appropriate types if they exist
        depths = torch.from_numpy(np.stack(batch["depths"]).astype(np.float32)) if "depths" in batch else None
        extrinsics = torch.from_numpy(np.stack(batch["extrinsics"]).astype(np.float32)) if "extrinsics" in batch else None
        intrinsics = torch.from_numpy(np.stack(batch["intrinsics"]).astype(np.float32)) if "intrinsics" in batch else None
        cam_points = torch.from_numpy(np.stack(batch["cam_points"]).astype(np.float32)) if "cam_points" in batch else None
        world_points = torch.from_numpy(np.stack(batch["world_points"]).astype(np.float32)) if "world_points" in batch else None
        point_masks = torch.from_numpy(np.stack(batch["point_masks"])) if "point_masks" in batch else None
        
        # 修正: batch["ids"] 可能是 Python list，torch.from_numpy 不吃 list，改用 torch.tensor
        ids = torch.tensor(batch["ids"])    # Frame indices sampled from the original sequence
        ...
        if depths is not None: sample["depths"] = depths
        if extrinsics is not None: sample["extrinsics"] = extrinsics
        if intrinsics is not None: sample["intrinsics"] = intrinsics
        if cam_points is not None: sample["cam_points"] = cam_points
        if world_points is not None: sample["world_points"] = world_points
        if point_masks is not None: sample["point_masks"] = point_masks
        if "inverse_materials" in batch: sample["inverse_materials"] = batch["inverse_materials"]
        if "available_gts" in batch: sample["available_gts"] = batch["available_gts"]
        if "dataset_name" in batch: sample["dataset_name"] = batch["dataset_name"]

        # Dynamically pass ground truth data with specific shape (for models like inverse rendering)
        for k in batch.keys():
            if (k.startswith("gt_") or k.startswith("mask_")) and batch[k] is not None:
                if isinstance(batch[k], list) and len(batch[k]) > 0 and isinstance(batch[k][0], np.ndarray):
                    sample[k] = torch.from_numpy(np.stack(batch[k]).astype(np.float32)).contiguous()
                else:
                    sample[k] = batch[k]
...
class TupleConcatDataset(ConcatDataset):
    ...
    """
    def __init__(self, datasets, common_config, weights=None):
        """
        ...
        if self.weights is not None:
            ...
            self.probs = [w / w_sum for w in self.weights]
        else:
            self.probs = None
    ...
    def __getitem__(self, idx):
        ...
        if self.inside_random:
            if self.probs is not None:
                # Weighted random sampling of dataset
                dataset_idx = random.choices(range(len(self.datasets)), weights=self.probs, k=1)[0]
                # Then pick a random item from that dataset
                ds_len = len(self.datasets[dataset_idx])
                sample_idx = random.randint(0, ds_len - 1)
                
                # We need to compute a "global" idx that points to this item in this dataset
                # to satisfy the rest of the logic or just return it directly.
                # Actually, the rest of the logic below finds dataset_idx and sample_idx from idx.
                # If we have them already, we can bypass.
                
                if len(idx_tuple) == 3:
                    new_idx_tuple = (sample_idx,) + idx_tuple[1:]
                else:
                    raise ValueError("Tuple index must have exactly three elements")
                return self.datasets[dataset_idx][new_idx_tuple]
            else:
                # Uniform random sampling across all samples
